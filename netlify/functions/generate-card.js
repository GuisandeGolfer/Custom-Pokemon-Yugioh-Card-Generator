...
async function generateCardText({ cardStyle, eventName, eventDate, location, people, theme, elementType }) {
...
async function generateCardImage({ cardStyle, eventName, location, theme, elementType }) {
  const styleLabel = cardStyle === 'pokemon' ? 'Pokémon' : 'Yu-Gi-Oh!';
...
export default async (req, context) => {
  ...
  try {
    const { cardStyle, eventName, eventDate, location, people, theme, elementType } = await req.json();

    if (!eventName || !cardStyle) {
      return new Response(JSON.stringify({ error: 'cardStyle and eventName are required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify({ ...textData, imageBase64, elementType }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (err) {
    console.error('Generation error:', err);
    return new Response(JSON.stringify({ error: err.message || 'Failed to generate card' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
